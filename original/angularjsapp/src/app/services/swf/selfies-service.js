...
.factory('selfies', function($interval, $timeout, config, Api, upload) {
	...
	selfies.loadSelfie = function(data, channelId) {
		...
		selfies.selfieImage.onload = function() {
			...
			selfies.selfieDelayedAnnounce = $timeout(function() {
				var sfbPieces = data.sfb.split('/');
				selfies.announceSelfie(channelId, channelId, data.userId, sfbPieces[sfbPieces.length - 2] + '/' + sfbPieces[sfbPieces.length - 1]);
			}, config.settings.SelfiePusherDelay);
		};
		selfies.selfieImage.src = data.sfb;
	};
				...
				if (response.data && response.data.list.length > 0) {
					...
					if (selfiesQueue.length > 0) {
						selfies.loadSelfie({
							...
							sfb: config.settings.SelfieBaseUrl + selfiesQueue[0].sfb,
	...
	selfies.notifyLogin = function(data) {
		...
		if (data.selfie) {
			var localsf = JSON.parse(data.selfie)['sf'];
			var localsfb = JSON.parse(data.selfie)['sfb'];
			selfies.localSelfieSrc.sf = config.settings.SelfieBaseUrl + localsf;
			selfies.localSelfieSrc.sfb = config.settings.SelfieBaseUrl + localsfb;
		}
	};
	...
	selfies.newSelfie = function(selfie) {
		selfie.selfiePath = config.settings.SelfieBaseUrl + selfie.selfiePath;
	...
	function clearLocalSelfie() {
		selfies.localSelfieSrc.preview = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVQYV2NgYAAAAAMAAWgmWQ0AAAAASUVORK5CYII=";
		selfies.localSelfieSrc.sf = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVQYV2NgYAAAAAMAAWgmWQ0AAAAASUVORK5CYII=";
		selfies.localSelfieSrc.sfb = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVQYV2NgYAAAAAMAAWgmWQ0AAAAASUVORK5CYII=";
	}
